...
{
    IM_ASSERT(idx <= obj->CurLenA);
    return obj->TextA[idx];
}
static float STB_TEXTEDIT_GETWIDTH(ImGuiInputTextState *obj, int line_start_idx, int char_idx)
{
    unsigned int c;
    ImTextCharFromUtf8(&c, obj->TextA.Data + line_start_idx + char_idx, obj->TextA.Data + obj->TextA.Size);
    if ((ImWchar)c == '\n')
...
static int is_word_boundary_from_right(ImGuiInputTextState *obj, int idx)
{
    ...
    unsigned int curr_c;
    ImTextCharFromUtf8(&curr_c, curr_p, obj->TextA.Data + obj->TextA.Size);
    unsigned int prev_c;
    ImTextCharFromUtf8(&prev_c, prev_p, obj->TextA.Data + obj->TextA.Size);
...
static int is_word_boundary_from_left(ImGuiInputTextState *obj, int idx)
{
    ...
    const char *prev_p = ImTextFindPreviousUtf8Codepoint(obj->TextA.Data, curr_p);
    unsigned int prev_c;
    ImTextCharFromUtf8(&prev_c, curr_p, obj->TextA.Data + obj->TextA.Size);
    unsigned int curr_c;
    ImTextCharFromUtf8(&curr_c, prev_p, obj->TextA.Data + obj->TextA.Size);
...
{
    Stb->cursor = ImMin(Stb->cursor, CurLenA);
    Stb->select_start = ImMin(Stb->select_start, CurLenA);
    Stb->select_end = ImMin(Stb->select_end, CurLenA);
}
...
{
    Stb->select_start = 0;
    Stb->cursor = Stb->select_end = CurLenA;
...
bool ImGui::Selectable(const char *label, bool selected, ImGuiSelectableFlags flags, const ImVec2 &size_arg)
{
    ...
    if (is_visible)
    {
        ...
        if (highlighted || selected)
        {
            ...
            ImU32 col = GetColorU32((held && highlighted) ? ImGuiCol_HeaderActive : highlighted ? ImGuiCol_HeaderHovered
                                                                                                : ImGuiCol_Header);
